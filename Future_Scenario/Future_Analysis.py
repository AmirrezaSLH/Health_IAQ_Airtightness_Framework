...
def plot_boxplots(data, title, xlabels, ylabel, colors='skyblue', save=False, filename='boxplot.png'):
    ...
    # Check if colors is a single color and expand it to the length of data if needed
    if isinstance(colors, str):
        colors = [colors] * len(data)  # Ensure we have one color per data set if only one color was provided
    ...
    # Set colors and properties for each box
    for patch, color in zip(box['boxes'], colors):
        patch.set_facecolor(color)  # Set each box's face color
...
plot_boxplots(sfd1_list, title = r'', xlabels = ['Vintage 1', 'Vintage 2', 'Vintage 3'], ylabel = r'Reduction in Outdoor-origin $PM_{2.5}$ ($\mu g/m^3$)', colors='skyblue', save=False, filename='boxplot.png')

sfb0_list = process_sf_conc(sfb0)

tp0 = [ x for x in (sfb0_list[0]) ]
tp1 = [ x for x in (sfb0_list[1]) ]
tp2 = [ x for x in (sfb0_list[2]) ]

sfb01_list = (tp0, tp1, tp2)
plot_boxplots(sfb01_list, title = r'Baseline Indoor $PM_{2.5}$ of Outdoor-origin', xlabels = ['Vintage 1', 'Vintage 2', 'Vintage 3'], ylabel = r'$\mu g/m^3$', colors='skyblue', save=False, filename='boxplot.png')

sf_bx_list = [sfb0_list, sfd1_list]
plot_grouped_boxplots2(sf_bx_list, 'Baseline and Reductions in Indoor $PM_{2.5}$ of Outdoor-origin', ['Baseline', 'Reductions'], ['Vintage 1', 'Vintage 2', 'Vintage 3'], r'$\mu g/m^3$')

sfi0_list = process_sf_conc(sfi0)

tpp0 = [ x for x in (sfi0_list[0]) ]
tpp1 = [ x for x in (sfi0_list[1]) ]
tpp2 = [ x for x in (sfi0_list[2]) ]
sfi0_list = (tpp0, tpp1, tpp2)
plot_boxplots(sfi0_list, title = r'Intervention Indoor $PM_{2.5}$ of Outdoor-origin', xlabels = ['Vintage 1', 'Vintage 2', 'Vintage 3'], ylabel = r'$\mu g/m^3$', colors='skyblue', save=False, filename='boxplot.png')

sf_bx2_list = [sfb0_list, sfi0_list, sfd1_list]
plot_grouped_boxplots2(sf_bx2_list, 'Indoor $PM_{2.5}$ of Outdoor-origin', ['Baseline', 'Alternative', 'Reductions'], ['Vintage 1', 'Vintage 2', 'Vintage 3'], r'$\mu g/m^3$')
