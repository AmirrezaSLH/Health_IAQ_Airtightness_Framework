...
def mean_concentration_indoor_calculator(phi, c):
    """
    ...
    residences_segment = phi.keys()  # Building sections
    
    for j in residences_segment:
        C_in[j] = c[j]  # Calculate indoor concentration
        mean_C_in += C_in[j] * phi[j]  # Weighted sum of indoor concentrations
        
    return mean_C_in


def delta_exposure_calculator(t_in, phi, c_baseline, c_intervention):
    """
    ...
    #This Function should be reformulated not to depend on FINF it is not comprehensive! 
    #It can not account for changes in lambda
    
    # Calculate mean indoor concentrations for baseline and intervention scenarios
    mean_C_in_baseline = mean_concentration_indoor_calculator(phi, c_baseline)
    mean_C_in_intervention = mean_concentration_indoor_calculator(phi, c_intervention)
    
    # Compute the difference in mean indoor concentrations due to the intervention
    delta_mean_C_in =  mean_C_in_baseline - mean_C_in_intervention
    #print(delta_mean_C_in)
    # Calculate the change in exposure over the specified time period
    delta_exposure = t_in * delta_mean_C_in
    
    return delta_mean_C_in
...
def outdoor_source_calculator(lam, P, K, C_out):
    #Should be completed based on need
        
    # Calculate outdoor source contamination
    outdoor_source = (C_out * P * lam) / (lam + K)
    
    return outdoor_source

def indoor_source_calculator(E, V, capture_efficiency, lam, K):
    #Should be completed based on need
        
    # Calculate indoor source contamination
    indoor_source = ( (E * (1 - capture_efficiency)) / V) / (lam + K)
    
    return indoor_source

def indoor_sum_calculator(t_cook, lam_range, lam_natural, E, V, capture_efficiency, P, K, C_out):
    
    lam_new = lam_range + lam_natural
    #print(lam_range)
    indoor_source = t_cook * indoor_source_calculator(E, V, capture_efficiency, lam_new, K)
    outdoor_source = (t_cook * outdoor_source_calculator(lam_new, P, K, C_out)) + ((1-t_cook) * outdoor_source_calculator(lam_natural, P, K, C_out))
    
    c_in = indoor_source + outdoor_source
...
def code_comply_all( ach50_segment_dict, GRID_KEY, climatezone_grid_map, P_d, K_d):
    ...
    #print(Finf_intervention['SF_V1'])
    #print(ACH_intervention['SF_V1'])
    return ACH_intervention


def initialize_cooking():
    
    t_cook = 0.042
    #t_cook = 0
    flow_range = 350 * 1.69901082
    #lam_range = 0
    E = 158400 
    #E = 0
    ...
    return (t_cook, E, flow_range, capture_efficiency)

'''
test = init_ACH50()
test2 = sample_ach50_mean(test)  
test3 = segment_matching( test2['(3, 7)'] )
test_in = {key: indoor_sum_calculator(0, 0, (value/16), 0, 1, 1, 0.72, 0.39, 10) for key, value in test3.items()}

climatezone_grid_map = init_climatezone_grid_map()
test_int = code_comply_all(test3, '(3, 7)', climatezone_grid_map, 0.72, 0.39)
test_in_int = {key: indoor_sum_calculator(t_cook, lam_range, (value/16), E, V, capture_efficiency, P_random, K_random, C_out) for key, value in ACH_intervention.items()}
'''

def run_MCS(intervention_function = code_comply_all, cooking_params = initialize_cooking() ,  HF = 'HF_3', rr_beta = 0.005826891, rr_beta_sd = 0.000962763, iterations = 10):
    #pm_dict = init_PM_Concentrations_means()
    ...
    t_cook, E, flow_range, capture_efficiency = cooking_params
    ...
    for n in range(iterations):
        ...
        for g in grid_key_list:
            ...
            indoor_c_baseline = {key: indoor_sum_calculator(t_cook, flow_range / (flarea_segment_dict[key]*0.092903) , (value/16), E, V, capture_efficiency, P_random, K_random, C_out) for key, value in ach50_segment_dict.items()}

            #Dictionary of Finf and ACH after intervention
            ACH_intervention = intervention_function(ach50_segment_dict, g, climatezone_grid_map, P_random, K_random)
            
            #bug was due to dividing an extra 16
            indoor_c_intervention = {key: indoor_sum_calculator(t_cook, flow_range / (flarea_segment_dict[key]*0.092903), value, E, V, capture_efficiency, P_random, K_random, C_out) for key, value in ACH_intervention.items()}
            ...
            delta_mean_C_in[g] = delta_exposure_calculator(t_in, phi, indoor_c_baseline, indoor_c_intervention)
    ...
    return list_delta_mean_C_in, list_dMort, list_benefit, list_dY

#c_mean, mort, ben, y = run_MCS(iterations =100)
